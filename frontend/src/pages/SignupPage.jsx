...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLocalError("");
    if (password !== confirm) {
      setLocalError(t("signup.mismatch"));
      return;
    }
    ...
    <div className="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mt-2">{t("signup.title")}</h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          {displayError && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700 flex justify-between items-center">
              ...
              <button onClick={() => { clearError(); setLocalError(""); }} className="text-red-400 hover:text-red-600 ml-2">&times;</button>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t("signup.confirm")}</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input
                  type={showPw ? "text" : "password"}
            ...
            <>
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="bg-white px-3 text-gray-400 uppercase tracking-wide">{t("signup.orContinue")}</span>
                </div>
              </div>
              <GoogleSignInButton onSuccess={handleGoogleSuccess} text={t("signup.googleBtn")} />
            </>
          )}
