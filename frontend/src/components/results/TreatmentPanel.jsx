  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-5 mt-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-gray-900">Treatment Advice</h3>
        <div className="flex gap-2">
          ...
          <button onClick={handleCopy}
            className="p-2 hover:bg-gray-100 rounded-lg text-gray-500" title="Copy">
            <ClipboardCopy className="w-4 h-4" />
          </button>
          {onListenClick && (
            <button onClick={onListenClick}
              className="p-2 hover:bg-gray-100 rounded-lg text-gray-500" title="Listen">
              <Volume2 className="w-4 h-4" />
            </button>
          )}
        </div>
      </div>
      {copied && <p className="text-green-600 text-xs mb-2">Copied to clipboard!</p>}
